...
def test_application_mode_uses_configured_application_token(monkeypatch, tmp_path):
    monkeypatch.setenv("HH_TOKEN_FILE", str(tmp_path / "tokens.json"))
    monkeypatch.setenv("HH_APPLICATION_TOKEN", "app-token")
    monkeypatch.setenv("HH_VACANCY_AUTH_MODE", "application")
    api = HHApiClient()
    captured = {}
...
def test_application_token_is_cached_to_file(monkeypatch, tmp_path):
    token_file = tmp_path / "app.json"
    monkeypatch.setenv("HH_TOKEN_FILE", str(tmp_path / "user.json"))
    monkeypatch.setenv("HH_APPLICATION_TOKEN_FILE", str(token_file))
    monkeypatch.setenv("HH_VACANCY_AUTH_MODE", "application")
    ...
    async def run():
        token = await api._get_application_token(FakeClient())
        assert token == "app-token"

    asyncio.run(run())
    assert token_file.exists()
    assert 'app-token' in token_file.read_text(encoding="utf-8")
